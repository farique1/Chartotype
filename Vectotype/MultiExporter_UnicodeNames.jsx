...
getSvgOptions = function (unicode_name, transparency, scaling, embedImage, embedFont, trimEdges ) {
...
var multi_exporter = {
	...
	load_prefs: function() {
		...
		try {
			...
			saved_data.appendChild( new XML('<nyt_unicode_name>true</nyt_unicode_name>') );
		...
		if ( this.multiExporterPrefs.textFrames.length != 1 ) {
			...
			try {
				...
				this.unicode_name   = this.prefs_xml.nyt_unicode_name == "true" ? true : false;
	...
	show_dialog: function() {
		...
		this.uniNmbrCheckBox = row.add('checkbox', undefined, 'Unicode Number Only');
		this.uniNmbrCheckBox.value = this.unicode_name;
	...
	saveOptions:function(){
		...
		this.unicode_name = this.uniNmbrCheckBox.value;
		...
		this.prefs_xml.nyt_unicode_name = this.unicode_name;
	...
	run_export: function() {
		...
		var num_exported = 0;
		var options = formatInfo.getOptions(this.unicode_name, this.transparency, this.scaling, this.embedImage, this.embedFont, this.trimEdges);
		...
		for (var i = 0; i < this.export_artboards.length; i++ ) {
			...
			if ( this.whole_artboard_mode) {
				...
				try{
					...
					// if (unicode_name){
					// 	unicode_number = artboardName.split("-")[0]
					// 	var base_filename = this.base_path + "/" + unicode_number;
					// }
					if(copyBehaviour){
				...
				for ( var j=0; j < this.export_layers.length; j++ ) {
					...
					try{
						...
						if (layerRect[0]<layerRect[2] && layerRect[1]>layerRect[3]) {
							...
							if (this.unicode_name){
								unicode_number = lyr_name.split(" - ")[0]
								var base_filename = this.base_path + "/" + unicode_number;
							}
							if(copyBehaviour){
